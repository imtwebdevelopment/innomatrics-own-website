...
const BlogDetail = () => {
  ...
  useEffect(() => {
    ...
    if (post && post.status === 'Published') {
      ...
      // Get 3 other published blogs for the sidebar
      ...
      // If not found or draft, redirect back to blogs page
      navigate('/blog');
    }
  }, [slug, navigate]);
            ...
            <div className="divide-y divide-slate-100 space-y-4">
              {recentBlogs.length > 0 ? (
                recentBlogs.map((post) => (
                  <div key={post.id} className="flex items-start gap-3.5 pt-4 first:pt-0">
                    <img
                      src={post.image}
                      alt={post.title}
